...
#pragma omp parallel shared(w) private(i, j) num_threads(num_thread)
  {
#pragma omp for
    for (i = 1; i < M - 1; i++)
    {
...
*/
#pragma omp parallel shared(mean, w) private(i, j) num_threads(num_thread)
  {
#pragma omp for
    for (i = 1; i < M - 1; i++)
    {
...
*/
  ...
  while (epsilon <= diff)
  {
#pragma omp parallel shared(u, w) private(i, j) num_threads(num_thread)
    {
/*
...
*/
    diff = 0.0;
#pragma omp parallel shared(diff, u, w) private(i, j, my_diff) num_threads(num_thread)
    {
      my_diff = 0.0;
#pragma omp for
      for (i = 1; i < M - 1; i++)
