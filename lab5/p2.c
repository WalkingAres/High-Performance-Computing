...
#pragma omp for reduction(+ \
    ...
    while (epsilon <= diff)
    {
        ...
        MPI_Allgather(u + my_index.first * N, (my_index.last - my_index.first) * N, MPI_DOUBLE, u + my_index.first * N, (my_index.last - my_index.first) * N, MPI_DOUBLE, MPI_COMM_WORLD);
